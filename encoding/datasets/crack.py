...
class CrackSegmentation(BaseDataset):
    BASE_DIR = 'crack'
    NUM_CLASS = 2

    # /data/deeplearning/ead2020_semantic_segmentation
    def __init__(self, root='/share/Dataset/db_test', split='train',
                 mode=None, transform=None, target_transform=None, base_size=768, crop_size=512, **kwargs):
        super(CrackSegmentation, self).__init__(
...
def _get_cityscapes_pairs(folder, split='train'):
    ...
    if split == 'train':
        split_f = os.path.join('datasets/crack', 'train_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'val':
        split_f = os.path.join('datasets/crack', 'val_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'test':
        split_f = os.path.join('datasets/crack', 'val_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    else:
        split_f = os.path.join('datasets/crack', 'trainval_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
