...
from .base import BaseDatasetV4
import tifffile as tif
import cv2

class BuildingSegmentation(BaseDatasetV4):
    # BASE_DIR = 'cityscapes'
    NUM_CLASS = 2
...
class BuildingSegmentation2(BaseDatasetV4):
    # BASE_DIR = 'cityscapes'
    NUM_CLASS = 2
    def __init__(self, root='/data/Dataset/buildings/crop_data/', split='train',
                 mode=None, transform=None, target_transform=None, **kwargs):
...
def _get_cityscapes_pairs(folder, split='train'):
    ...
    if split == 'train':
        split_f = os.path.join('bdnet/buildings/datasets/buildings', 'train_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'val':
        split_f = os.path.join('bdnet/buildings/datasets/buildings', 'val_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'trainval':
        split_f = os.path.join('bdnet/buildings/datasets/buildings', 'trainval_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)       
    # elif split == 'test':
