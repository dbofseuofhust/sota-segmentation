...
from .base import BaseDataset,BaseDatasetV3


class DiseaseSegmentation(BaseDatasetV3):
    BASE_DIR = 'disease'
    NUM_CLASS = 6

    # /data/deeplearning/ead2020_semantic_segmentation
    def __init__(self, root='/data/Dataset/EDD2020_release-I_2020-01-15', split='train',
                 mode=None, transform=None, target_transform=None, base_size=768, crop_size=512, **kwargs):
        super(DiseaseSegmentation, self).__init__(
...
def _get_cityscapes_pairs(folder, split='train'):
    ...
    if split == 'train':
        split_f = os.path.join('datasets/disease', 'train_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'val':
        split_f = os.path.join('datasets/disease', 'val_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'test':
        split_f = os.path.join('datasets/disease', 'val_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    else:
        split_f = os.path.join('datasets/disease', 'trainval_fine.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
