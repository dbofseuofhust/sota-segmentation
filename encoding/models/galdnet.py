...
# https://arxiv.org/pdf/1909.07229.pdf
...
class GALDNet(BaseNet):
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, avg=False, **kwargs):
        # ori
        # self.inplanes = 128

        # new modify by db to load imagenet pretrain
        self.inplanes = 64
        super(GALDNet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        ...
        self.head = GALDHead(2048, 512, 512, num_classes=nclass)
    ...
    def forward(self, x):
        ...
        # new modified to unify with base.py and mutilscale eval
...
def get_galdnet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = GALDNet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
