...
from .ema_module.nn.modules import SynchronizedBatchNorm2d
...
class EMANet(nn.Module):
    ...
    def __init__(self, n_classes, backbone, stride, stage_num):
        super().__init__()
        backbone = resnet(backbone, stride)
    ...
    def forward(self, img, size=None):
        ...
        if size is None:
            size = img.size()[-2:]
        preds = F.interpolate(x, size=size, mode='bilinear', align_corners=True)
...
def get_emanet(dataset='pascal_voc', backbone='resnet50', stride=8, stage_num=3, pretrained=False,
    ...
    model = EMANet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, stride=stride, stage_num=stage_num)
    return model
