...
from encoding.models.base import ChopEvalModule
...
def test(args):
    ...
    root_dir = '/data/Dataset/buildings/crop_data/'
    if args.eval:
        ...
        #                                    transform=input_transform,root=root_dir)
        root_dir = '/data/Dataset/buildings/buildings_testA/'
        root_dir = args.test_folder
    ...
    if args.model_zoo is not None:
        ...
        model = get_model(args.model_zoo, pretrained=False)
    ...
    evaluator = ChopEvalModule(model, testset.num_class).cuda()
    evaluator.eval()
    ...
    def eval_batch(image, dst, evaluator, eval_mode):
        if eval_mode:
            ...
            for predict, impath in zip(predicts, im_paths):
                # import pdb;pdb.set_trace()
                if 1:
                    predict = predict.squeeze().astype('uint8')
                    kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(5,5)) #可以修改卷积核大小来增加腐蚀效果，越大腐蚀越强
                    predict = cv2.erode(predict,kernel)
                    
                    kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(3,3)) #可以修改卷积核大小来增加腐蚀效果，越大腐蚀越强
                    predict = cv2.dilate(predict,kernel)
