...
# split = ['train','val']
#
# root = r'/share/Dataset/ead2020_semantic_segmentation'
#
...
prerawroot = r'/share/Dataset/db_test'
split = ['train', 'val']
for v in split:
    imginfos = []
    maskinfos = []
    imgs = os.listdir(os.path.join(prerawroot, v, 'crack'))
    for w in imgs:
        imginfos.append(os.path.join(v, 'crack', w))
        maskinfos.append(os.path.join(v, 'mask', '{}.png'.format(w.split('.')[0])))

    os.makedirs('datasets/crack',exist_ok=True)

    with open(os.path.join('datasets/crack',"{}_fine.txt").format(v),'w') as f:
